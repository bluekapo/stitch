...
import { buildCurrentChunkTasksView, buildCurrentChunkView } from '../view-builders.js';
import { renderCurrentChunkTasksView, renderCurrentChunkView } from '../views.js';
import { safeEditMessageText } from './helpers.js';

export function createHubMenu(
	...
	const menu = new Menu<StitchContext>('hub')
		...
		.text('Tasks', async (ctx) => {
			...
			await safeEditMessageText(
				ctx,
				renderCurrentChunkTasksView(buildCurrentChunkTasksView(taskService, dailyPlanService)),
			);
		});
